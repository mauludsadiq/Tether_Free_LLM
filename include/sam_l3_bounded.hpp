...
public:
    ...
    void clear() {
        ...
        in_cnt.resize(st.size() * SamNode64::K, 0);
        h_cnt.resize(st.size() * SamNode64::H, 0);
        st[0].link = NIL;
        in_cnt.assign(SamNode64::K, 0);
        h_cnt.assign(SamNode64::H, 0);
        in_cnt.assign(SamNode64::K, 0);
        h_cnt.assign(SamNode64::H, 0);
        sz = 1;
        last = 0;
    ...
    void match_out_edges(std::vector<OutEdge>& out, uint32_t max_edges = 16) const {
        out.clear();
        if (m_state == NIL) return;
        uint32_t s = m_state;
        ...
        for (int j = 0; j < SamNode64::K; j++) {
            ...
            uint32_t to = st[s].in_to[j];
            if (to == NIL) continue;
            uint32_t idx = s * SamNode64::K + (uint32_t)j;
            uint32_t w = (idx < in_cnt.size()) ? (uint32_t)in_cnt[idx] : st[to].occ;
            OutEdge e{ k, to, w };
            out.push_back(e);
    ...
    void finalize_edge_counts() {
        if (st.empty()) return;
        in_cnt.resize(st.size() * SamNode64::K, 0);
        h_cnt.resize(st.size() * SamNode64::H, 0);
        for (uint32_t s = 0; s < (uint32_t)st.size(); s++) {
            for (int j = 0; j < SamNode64::K; j++) {
                uint32_t k = st[s].in_key[j];
                uint32_t to = st[s].in_to[j];
                if (k == EMPTY_KEY || to == NIL || to >= (uint32_t)st.size()) {
                    in_cnt[s * SamNode64::K + (uint32_t)j] = 0;
                    continue;
                }
                uint32_t w = (uint32_t)in_cnt[s * SamNode64::K + (uint32_t)j];
                if (w > 65535u) w = 65535u;
                in_cnt[s * SamNode64::K + (uint32_t)j] = (uint16_t)w;
            }
            for (int j = 0; j < SamNode64::H; j++) {
                uint32_t k = st[s].h_key[j];
                uint32_t to = st[s].h_to[j];
                if (k == EMPTY_KEY || to == NIL || to >= (uint32_t)st.size()) {
                    h_cnt[s * SamNode64::H + (uint32_t)j] = 0;
                    continue;
                }
                uint32_t w = (uint32_t)h_cnt[s * SamNode64::H + (uint32_t)j];
                if (w > 65535u) w = 65535u;
                h_cnt[s * SamNode64::H + (uint32_t)j] = (uint16_t)w;
            }
        }
    }
...
private:
    ...
    std::vector<uint16_t> in_cnt;
    std::vector<uint16_t> h_cnt;
    uint32_t sz = 0;
    uint32_t last = 0;
